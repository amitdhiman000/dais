...
class PollsResponse(models.Model):
	...
	def response_yes(klass, user, poll):
		# prior check
		pollObj = Poll(pk=poll)
		pollResObject = klass.objects.get_or_create(user=user, poll=pollObj)[0]
		if 1 != pollResObject.response:
			pollResObject.response = 1
			pollResObject.save()
	...
	@classmethod
	def response_notsure(klass, user, poll):
		# prior check
		poll_object = klass.objects.get_or_create(user=user, poll=poll)[0]
